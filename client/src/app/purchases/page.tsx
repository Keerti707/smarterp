...
type InventoryItem = { id: string; name: string; purchase_price: string; current_stock: string };
...
export default function PurchasesPage() {
  ...
  async function load(companyId: string) {
    const [purchaseData, supplierData, inventoryData] = await Promise.all([
      apiRequest(`/purchases/${companyId}`),
      apiRequest(`/suppliers/${companyId}`),
      apiRequest(`/inventory/${companyId}`),
    ]);

    setInvoices(purchaseData);
  ...
  async function createInvoice(event: React.FormEvent<HTMLFormElement>) {
    ...
    await apiRequest(`/purchases/${company.id}`, {
      method: "POST",
      body: JSON.stringify({
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-4xl font-black">Purchase Invoices</h1>
          </div>
              ...
              <form onSubmit={createInvoice} className="space-y-5">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="grid gap-2">
                    ...
                    <select value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="h-10 rounded-md border border-white/10 bg-background px-3">
                ...
                <div className="space-y-3">
                  {items.map((item, index) => (
                    <div key={index} className="grid gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:grid-cols-[1.5fr_0.6fr_0.7fr_0.6fr_auto]">
                      <div className="grid gap-2">
                        <Label>Item</Label>
                        <select value={item.inventory_item_id} onChange={(e) => updateItem(index, "inventory_item_id", e.target.value)} className="h-10 rounded-md border border-white/10 bg-background px-3" required>
                      ...
                      <div className="grid gap-2">
                        <Label>GST %</Label>
                        <Input type="number" value={item.gst_percent} onChange={(e) => updateItem(index, "gst_percent", e.target.value)} />
                      </div>

                      <Button type="button" variant="destructive" size="icon" className="mt-8" onClick={() => removeLine(index)} disabled={items.length === 1}>
        ...
        <Card className="border-white/10 bg-white/[0.03]">
          <CardContent className="p-5">
            <div className="relative">
              ...
              <Input placeholder="Search purchases..." className="pl-11" value={query} onChange={(e) => setQuery(e.target.value)} />
            </div>
          </CardContent>
        </Card>
              ...
              <Card key={invoice.id} className="border-white/10 bg-white/[0.03]">
                <CardContent className="flex items-center justify-between p-5">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{invoice.supplier_name ?? "Walk-in Supplier"}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{invoice.invoice_date}</p>
                  </div>

                  <div className="text-right">
                    <p className="text-lg font-bold">₹{Number(invoice.total_amount).toLocaleString("en-IN")}</p>
                    <p className="text-xs text-muted-foreground">GST ₹{Number(invoice.gst_amount).toLocaleString("en-IN")}</p>
                  </div>
                </CardContent>
              </Card>
            ))
